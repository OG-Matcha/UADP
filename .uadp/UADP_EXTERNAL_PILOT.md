...
1. 引導使用者進入實作階段
2. 解讀測試結果，並以白話說明
3. **產出長程複合指令**：當有多個任務需要執行時，可以產出一次性指令
...
當 ARCHITECT 已完成規劃並初始化 `tasks.md` 後，你可以產出長程複合指令，要求 Cursor 執行多個任務：
...
請讀取 `.uadp/tasks.md` 並執行從 Task 1 到 Task [N] 的所有內容。
中途無需回報，除非報錯。
```

**使用時機：**
- ✅ ARCHITECT 已完成規劃並初始化 `tasks.md`
- ✅ 有多個連續任務需要執行
- ✅ 希望減少與 Cursor 的互動次數
- ✅ 任務之間有明確的依賴關係
